...
function makeActionManager(env) {
    ...
    async function _triggerDownload(action, options, type) {
        ...
        var hash = window.location.hash.substring(1);
                ...
                const user_setting = await rpc.query({
                    model: 'res.users',
                    method: 'search_user_report_option',
                    args: [[], {}],
                });

                if (user_setting == 'print') {
                    env.services.ui.block(); // Block UI
                    // ---------------------------------------
                    // IF USER SET PRINT REPORT OPTIONS 
                    // --------------------------------------- 
                    $('#frame-pdf').remove();
                    
                    $('<iframe style="display: none;">').attr({ id: 'frame-pdf', src: url, name: 'frame-pdf' }).appendTo(document.body).on("load", function (responseText, textStatus, jqXHR) {
                        window.frames['frame-pdf'].focus();
                        setTimeout(function () {
                            window.frames['frame-pdf'].print();
                            
                            env.services.ui.unblock(); // Unblock UI
