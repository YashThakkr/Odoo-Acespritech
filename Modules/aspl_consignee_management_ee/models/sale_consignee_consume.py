    ...
    def create(self, vals):
        for rec in vals:
            rec['name'] = self.env['ir.sequence'].next_by_code(
                'aspl_consignee_management_ee.sequence_consignee_consume_items')
        ...
        if line_lst:
            ...
            if sale_id:
                ...
                create_sale_state = self.env['ir.config_parameter'].sudo().get_param(
                    'aspl_consignee_management_ee.is_create_sale_order')
                if create_sale_state:
                    sale_id.action_confirm()
                send_mail = self.env['ir.config_parameter'].sudo().get_param(
                    'aspl_consignee_management_ee.is_send_mail')
                if send_mail:
                    template_id = self.env.ref(
                        'aspl_consignee_management_ee.consume_items_order_template')
