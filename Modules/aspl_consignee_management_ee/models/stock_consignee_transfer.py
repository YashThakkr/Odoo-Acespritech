...
class StockConsigneeTransfer(models.Model):
    ...
    def default_get(self, field_list):
        if not self._context.get('default_is_return') and not self._context.get(
            ...
            if self.env.user.has_group('aspl_consignee_management_ee.group_stock_consignee'):
        ...
        for rec in vals:
            if rec.get('is_return'):
                rec['name'] = self.env['ir.sequence'].next_by_code(
                    'aspl_consignee_management_ee.sequence_consignee_return_transfer')
            elif rec.get('is_internal_transfer'):
                rec['name'] = self.env['ir.sequence'].next_by_code(
                    'aspl_consignee_management_ee.sequence_consignee_internal_transfer')
            else:
                rec['name'] = self.env['ir.sequence'].next_by_code(
                    'aspl_consignee_management_ee.sequence_consignee_transfer')
    ...
    def action_request(self):
        ...
        if self.env['ir.config_parameter'].sudo().get_param(
                'aspl_consignee_management_ee.is_send_mail_request'):
            if self.is_return:
                ...
                if stock_manager_group and stock_manager_group.users:
                    ...
                    if emails:
                        template_id = self.env.ref(
                            'aspl_consignee_management_ee.consignee_request_template')
                        template_id.with_context(email_to=emails).send_mail(self.id,
                                                                            force_send=True)
            elif self.is_internal_transfer:
                template_id = self.env.ref(
                    'aspl_consignee_management_ee.consignee_consignee_request_internal_template')
                template_id.send_mail(self.id, force_send=True)
            else:
                template_id = self.env.ref(
                    'aspl_consignee_management_ee.user_consignee_request_template')
                template_id.send_mail(self.id, force_send=True)
        return True
    ...
    def action_confirm(self):
        ...
        if self.env['ir.config_parameter'].sudo().get_param(
                'aspl_consignee_management_ee.is_send_mail_confirm'):
            if self.is_return:
                if self.partner_id.id == self.env.user.partner_id.id and self.env.user.has_group(
                        'aspl_consignee_management_ee.group_stock_consignee'):
                    raise ValidationError(
                        _('Consignee can not confirm their own return transfer request...!'))
                template_id = self.env.ref('aspl_consignee_management_ee.consignee_confirm_template')
                template_id.send_mail(self.id, force_send=True)
            elif self.is_internal_transfer:
                template_id = self.env.ref(
                    'aspl_consignee_management_ee.consignee_consignee_confirm_internal_template')
                template_id.send_mail(self.id, force_send=True)
            else:
                stock_manager_group = self.env.ref('stock.group_stock_manager')
                if stock_manager_group and stock_manager_group.users:
                    ...
                    if emails:
                        template_id = self.env.ref(
                            'aspl_consignee_management_ee.user_consignee_confirm_template')
                        template_id.with_context(email_to=emails).send_mail(self.id,
                                                                            force_send=True)
        return True
    ...
    def get_product_expiry_date_scheduler(self):
        if self.env['ir.config_parameter'].sudo().get_param(
                'aspl_consignee_management_ee.is_send_alert_for_product_expired'):
            ...
            get_days = self.env['ir.config_parameter'].sudo().get_param(
                'aspl_consignee_management_ee.days')
            ...
            for con_data in consignee_locations:
                ...
                temp_id = self.env.ref('aspl_consignee_management_ee.product_expiry_alert_mail')
