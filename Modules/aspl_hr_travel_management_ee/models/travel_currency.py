...
class TravelCurrency(models.Model):
    ...
    def change_account(self):
        if self.journal_id:
            if self.journal_id.company_id.account_journal_payment_debit_account_id:
                self.account_id = self.journal_id.company_id.account_journal_payment_debit_account_id.id
            else:
                raise ValidationError(_("Please select the default accounts for the journal"))
        ...
        try:
            ...
            if value != False:
                ...
                if move_id.id:
                    ...
                    raise ValidationError(_("Journal Entry is not done"))
            else:
                raise ValidationError(_("Please Configure Account in the Employee Configuration"))
        except Exception:
            raise ValidationError(_("Please Configure Account in the Employee Configuration"))
