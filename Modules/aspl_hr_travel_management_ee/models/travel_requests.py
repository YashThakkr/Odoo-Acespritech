...
class EmployeeTravelReqest(models.Model):
    ...
    state = fields.Selection(
        [('draft', 'Draft'), ('waiting', 'Waiting for confirmation'), ('confirmed', 'Confirmed'), ('approved', 'Approved'), ('ongoing', 'Ongoing'),
         ('trip_completed', 'Trip Completed'), ('closed', 'Closed'), ('rejected', 'Rejected'), ('canceled', 'Canceled')], default="draft", tracking=True)
    # travel locations
    ...
    ticket_book = fields.Boolean(string="Company Books ticket for You", tracking=True)
    # Log
    ...
    def check_from_dates(self):
        if self.from_date:
            if self.from_date < date.today():
                raise ValidationError(_("Please enter  valid date greater than today("+str(date.today())+")"))
    ...
    def fill_by_grade(self):
        if self.grade_id:
            ...
            if expense_ids.expense_line_ids:
                ...
                raise ValidationError(_("Please Set the Grade Expenses in the  Configuration=>Expenses(Grade) Menu !!!"))
        else:
            self.expense_amount_ids = None
            raise ValidationError(_("Please Set the Grade in the Employee Details!!!"))
    ...
    def compute_days_travel_location(self):
        for each in self:
            if each.travel_location_ids:
                each.total_travel_days = sum([(i.travel_days) if i.travel_days else 0 for i in each.travel_location_ids])
            else:
                each.total_travel_days = 0
    ...
    def check_locations(self):
        ...
        if self.travel_location_ids:
            ...
            raise ValidationError(_("Please insert locations"))
    ...
    def check_date(self):
        self.ensure_one()
        if self.from_date < date.today():
            raise ValidationError(_("Please Enter Future Dates Past dates not allowed"))
        if self.from_date and self.to_date:
        ...
        if check_records or check_record_from_dates or check_record_to_dates:
            raise ValidationError(_("Already booked in " + str(self.from_date) + " to "+str(self.to_date)+"\n Select different dates"))
    ...
    def action_send_manager(self):
        ...
        if not self.travel_location_ids:
            raise ValidationError(_("Please enter the travel information before submit to manager"))
        if mail_address:
            try:
                template_id = self.env['ir.model.data'].get_object_reference('aspl_hr_travel_management_ee',
    ...
    def action_confirm(self):
        ...
        if self.env.uid in admin1.users.ids:
            if not self.travel_location_ids:
                raise ValidationError(_("Without place no request get Proceed."))
            self.sudo().state = 'confirmed'
        elif self.env.uid != self.employee_id.user_id.id:
            if not self.travel_location_ids:
                raise ValidationError(_("Without place request not proceed."))
            self.sudo().state = 'confirmed'
        else:
            raise ValidationError(_("You are not allow to confirm this request,\nplease contact to the administrator."))
    ...
    def action_approve(self):
        ...
        if (self.env.uid in hr_user.users.ids) or (self.env.uid in admin1.users.ids):
            ...
            raise ValidationError(_("You are not allow to approve trip,\n please contact to the administrator."))
        for currency_exp in self.currency_ids:
            if not currency_exp.account_id:
                raise ValidationError(_("Please check currency tab \nAccount is missing."))
    ...
    def action_close(self):
        for check in self.expenses_ids:
            if not (check.state in ['done', 'refused']):
                raise ValidationError(_("Verify expenses payment is remaining."))
        for proposed_exp in self.expense_amount_ids:
            if not proposed_exp.status:
                raise ValidationError(_("Please complete all proposed expenses."))
        for currency_exp in self.currency_ids:
            if not currency_exp.status:
                raise ValidationError(_("Please complete the payment of other currency."))
    ...
    def action_reject(self):
        view_id = self.env.ref("aspl_hr_travel_management_ee.aspl_reason_reject_wizard")
        return {
