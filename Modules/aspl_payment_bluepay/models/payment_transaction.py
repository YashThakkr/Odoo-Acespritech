...
class PaymentTxbluepay(models.Model):
    ...
    def _process_feedback_data(self, data):
        ...
        if self.provider != "bluepay":
            return
        self._set_done()
        self.with_context({'bluepay': True})._reconcile_after_done()

    def _reconcile_after_done(self):
        """ Override of payment to automatically confirm quotations and generate invoices. """
        if self._context.get('bluepay'):
                ...
                if default_template:
                    for trans in self.filtered(
                        ...
                        trans = trans.with_company(trans.acquirer_id.company_id).with_context(
                            ...
                            company_id=trans.acquirer_id.company_id,
                        )
