        ...
        if self._context.get('fitness_categ'):
            ...
            if self._context.get('gym_service') and 'company_id' in field_list:
                ...
                categ_dict['categ_id'] = self.env.ref(
                    'aspl_fitness_management_ee.gym_equipment_category').id
            elif self._context.get('fitness_membership') and 'company_id' in field_list:
                categ_dict['categ_id'] = self.env.ref(
                    'aspl_fitness_management_ee.gym_membership_category').id
...
class EquipmentMaintenance(models.Model):
    ...
    def button_sent_invoice(self):
        ...
        try:
            template_id = self.env.ref('aspl_fitness_management_ee.email_template_equipment_invoice')
        except ValueError:
            template_id = False
        try:
