...
class AccountRegisterPayments(models.TransientModel):
    ...
    def _create_payment_vals_from_batch(self, batch_result):
        ...
        if self.env.context.get('active_model') == 'account.move' and self.env.context.get(
            ...
            elif len(check_branch_diff) == 1 and move_ids.mapped('branch_id'):
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(AccountRegisterPayments, self).fields_view_get(view_id=view_id,
                                                                   view_type=view_type,
                                                                   toolbar=toolbar,
                                                                   submenu=submenu)
        if self._context.get('only_full_payment') and view_type == 'form':
