...
#                                                          data.get('form', False).get('branch_ids',
#                                                                                      False) else False
#         return res
...
#         data['form'] = self.read(
#             ['date_from', 'date_to', 'journal_ids', 'target_move', 'branch_ids', 'company_id'])[0]
...
#         query = 'SELECT SUM(debit) FROM ' + query_get_clause[
#             0] + ', account_move am WHERE "account_move_line".move_id=am.id AND am.state IN %s AND "account_move_line".journal_id IN %s AND ' + \
#                 query_get_clause[1] + ' '
#         if data['form']['branch_ids']:
#             query += """AND account_move_line.branch_id IN (%s)""" % (
#                 ','.join(map(str, data['form']['branch_ids'])))
...
#         query_get_clause = self._get_query_get_clause(data)
#         params = [tuple(move_state), tuple(journal_id.ids)] + query_get_clause[2]
#         query = 'SELECT SUM(credit) FROM ' + query_get_clause[
#             0] + ', account_move am WHERE "account_move_line".move_id=am.id AND am.state IN %s AND "account_move_line".journal_id IN %s AND ' + \
#                 query_get_clause[1] + ' '
#         if data['form']['branch_ids']:
#             query += """AND account_move_line.branch_id IN (%s)""" % (
