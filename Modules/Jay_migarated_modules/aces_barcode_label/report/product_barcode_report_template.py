...
class ProductBarcodeReportTemplate(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if 'barcode' in [x.name for x in self.env['aces.small.barcode.label.line'].browse(data['form']['field_lines'])] \
            ...
            for product in self.env['aces.small.product.label.qty'].browse(data['form']['product_lines']):
                ...
                try:
                    ...
                    raise UserError('Select valid barcode type according product barcode value !')
        ...
        return {
            ...
            'draw_image_style': self._draw_image_style,
            'data': data
        }
    ...
    def _get_label_data(self, form):
        ...
        for line in self.env['aces.small.barcode.label.line'].browse(form['field_lines']):
            selected_fields.update({int(line.sequence): line.name})
    ...
    def _draw_image_style(self, form):
        return "margin:5px;width:%s;height:%s;" % (str(form['image_width']) + "px", str(form['image_height']) + "px")
