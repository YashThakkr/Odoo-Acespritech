...
class ProductSmallFieldsLabel(models.AbstractModel):
        ...
        if field_line.field_id.name == 'attribute_value_ids':
            if product_id.attribute_ids:
                ...
                for attribute in product_id.prod_small_wiz_id.attribute_ids:
                    if attribute.id in [each.id for each in product_id.product_id.attribute_value_ids]:
                        field_value += attribute.attribute_id.name + ':' + attribute.name + ';'
            else:
                for attribute in product_id.product_id.attribute_value_ids:
                    field_value += attribute.attribute_id.name + ':' + attribute.name + ';'

        if field_line.field_id.name == 'categ_id':
            if product_id.product_id.categ_id:
                    field_value = product_id.product_id.categ_id.name
