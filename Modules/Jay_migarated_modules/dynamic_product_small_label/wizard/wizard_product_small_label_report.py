...
class AcesDesignFieldLine(models.TransientModel):
...
class WizardProductSmallLabelReport(models.TransientModel):
    ...
    def default_get(self, fields_list):
        prod_list = []
        product_list_dict = []
        res = super(WizardProductSmallLabelReport, self).default_get(fields_list)
        if self._context.get('active_ids') and self._context.get('active_model') == 'purchase.order':
            ...
            for line in self.env['account.move.line'].search([('move_id', 'in', self._context.get('active_ids'))]):
        ...
        elif self._context.get('active_ids') and self._context.get('active_model') == 'stock.lot':
            for each_lot in self.env['stock.lot'].browse(self._context.get('active_ids')):
                product_list_dict.append(
        ...
        for each_dict in product_list_dict:
            prod_list.append((0, 0, {
                'product_id': each_dict.get('product_id'),
    ...
    def _get_report_paperformat_id(self):
        xml_id = self.env['ir.actions.report'].search(
            [('report_name', '=', 'dynamic_product_small_label.prod_small_lbl_rpt')])
    ...
    label_height = fields.Integer(string='Label Height (mm)', default=40, required=True)
    ...
    def save_design(self):
        if not self.make_update_existing:
            view_id = self.env.ref('dynamic_product_small_label.wizard_product_small_label_design_form_view').id
    ...
    def action_print(self):
        ...
        if self.design_using == 'xml_design':
            ...
            return self.env.ref('dynamic_product_small_label.dynamic_product_page_report').report_action(self,
                                                                                                         data=datas)
        ...
        if skip_products:
            ...
            if group_id and group_id.users:
                mail = self.env['mail.mail'].sudo().create({
                    ...
                    'state': 'outgoing',
                    'recipient_ids': [(4, self.env.user.partner_id.id)]
                })
                ...
                body += """</table>"""
                values = {
                    'record_name': 'Dynamic Small Label module',
                ...
                notification_id = self.env['mail.notification'].sudo().create({
...
class ProductLabelQty(models.TransientModel):
...
class AcesProductFieldLine(models.TransientModel):
