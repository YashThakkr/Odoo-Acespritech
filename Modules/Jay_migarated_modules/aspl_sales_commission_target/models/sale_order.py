...
class SaleConfiguration(models.TransientModel):
    ...
    def get_values(self):
        ...
        res.update({
            'commission_pay_on': param_obj.sudo().get_param(
                'aspl_sales_commission_target.commission_pay_on'),
            'commission_pay_by': param_obj.sudo().get_param(
                'aspl_sales_commission_target.commission_pay_by'),
            'commission_included_amount': param_obj.sudo().get_param(
                'aspl_sales_commission_target.commission_included_amount'),
            'dist_commission_pay_on': param_obj.sudo().get_param(
                'aspl_sales_commission_target.dist_commission_pay_on'),
            'dist_commission_pay_by': param_obj.sudo().get_param(
                'aspl_sales_commission_target.dist_commission_pay_by'),
            'dist_commission_included_amount': param_obj.sudo().get_param(
                'aspl_sales_commission_target.dist_commission_included_amount'),
            'cons_commission_pay_on': param_obj.sudo().get_param(
                'aspl_sales_commission_target.cons_commission_pay_on'),
            'cons_commission_pay_by': param_obj.sudo().get_param(
                'aspl_sales_commission_target.cons_commission_pay_by'),
            'cons_commission_included_amount': param_obj.sudo().get_param(
                'aspl_sales_commission_target.cons_commission_included_amount'),
        })
        return res
    ...
    def set_values(self):
        ...
        param_obj = self.env['ir.config_parameter']
        param_obj.sudo().set_param('aspl_sales_commission_target.commission_pay_on',
                                   self.commission_pay_on)
        param_obj.sudo().set_param('aspl_sales_commission_target.commission_pay_by',
                                   self.commission_pay_by)
        param_obj.sudo().set_param('aspl_sales_commission_target.commission_included_amount',
                                   self.commission_included_amount)
        param_obj.sudo().set_param('aspl_sales_commission_target.dist_commission_pay_on',
                                   self.dist_commission_pay_on)
        param_obj.sudo().set_param('aspl_sales_commission_target.dist_commission_pay_by',
                                   self.dist_commission_pay_by)
        param_obj.sudo().set_param('aspl_sales_commission_target.dist_commission_included_amount',
                                   self.dist_commission_included_amount)
        param_obj.sudo().set_param('aspl_sales_commission_target.cons_commission_pay_on',
                                   self.cons_commission_pay_on)
        param_obj.sudo().set_param('aspl_sales_commission_target.cons_commission_pay_by',
                                   self.cons_commission_pay_by)
        param_obj.sudo().set_param('aspl_sales_commission_target.cons_commission_included_amount',
                                   self.cons_commission_included_amount)

    commission_pay_on = fields.Selection([('order_confirm', 'Sales Order Confirmation'),
                                          ('invoice_validate', 'Customer Invoice Validation'),
                                          ('invoice_pay', 'Customer Invoice Payment')],
                                         string="Sales Pay On")
    commission_pay_by = fields.Selection([('invoice', 'Invoice'), ('salary', 'Salary')],
                                         string="Sales Pay By")
    commission_included_amount = fields.Selection([('with_tax', 'With Tax'),
                                                   ('without_tax', 'Without Tax')],
                                                  string="Sales Included Amount",
                                                  default='without_tax')
    dist_commission_pay_on = fields.Selection([('order_confirm', 'Sales Order Confirmation'),
                                               ('invoice_validate', 'Customer Invoice Validation'),
                                               ('invoice_pay', 'Customer Invoice Payment')],
                                              string="Distributor Pay On")
    dist_commission_pay_by = fields.Selection([('invoice', 'Invoice'), ('salary', 'Salary')],
                                              string="Distributor Pay By")
    dist_commission_included_amount = fields.Selection([('with_tax', 'With Tax'),
                                                        ('without_tax', 'Without Tax')],
                                                       string="Distributor Included Amount",
                                                       default='without_tax')
    cons_commission_pay_on = fields.Selection([('order_confirm', 'Sales Order Confirmation'),
                                               ('invoice_validate', 'Customer Invoice Validation'),
                                               ('invoice_pay', 'Customer Invoice Payment')],
                                              string="Consultant Pay On")
    cons_commission_pay_by = fields.Selection([('invoice', 'Invoice'), ('salary', 'Salary')],
                                              string="Consultatn Pay By")
    cons_commission_included_amount = fields.Selection([('with_tax', 'With Tax'),
                                                        ('without_tax', 'Without Tax')],
                                                       string="Consultant Included Amount",
                                                       default='without_tax')
...
class SaleOrder(models.Model):
    ...
    def _prepare_invoice(self):
        ...
        if res:
            if self.commission_role_ids:
                res.update({'commission_calc': self.commission_calc or '', 'commission_role_ids': [
                    (6, 0, ([each.id for each in self.commission_role_ids]))]})
            if self.consultant_ids:
                res.update({'consultant_ids': [
                                (6, 0, ([each.id for each in self.consultant_ids]))]})
        return res
    ...
    def action_cancel(self):
        ...
        for saleid in self:
            comm_obj.search([('state', '=', 'draft'),
                             ('name', '=', saleid.name)]).write({'state': 'cancel'})
        return res

    def order_calculate_commission(self, ptype, commission_pay_by):
        comm_obj = self.env['sales.commission']
        for commline in self.sale_order_comm_ids.\
                filtered(lambda l: l.commission_type == ptype):
            vals = {
                'name': self.name,
                'sale_id': self.id,
            ...
            total_paid_amount = sum(
                comm_ids.filtered(lambda cid: cid.state == 'paid' or
                                              cid.invoice_id).mapped('amount'))
            if total_paid_amount <= commline.commission:
                vals['amount'] = commline.commission - total_paid_amount
            total_sales_amount = sum(
                comm_ids.filtered(lambda cid: cid.state == 'paid' or
                                              cid.invoice_id).mapped('user_sales_amount'))
    ...
    def order_calculate_commission_cons_dist(self, ptype, commission_pay_by, invoice_id):
        comm_obj = self.env['sales.commission']
        for commline in invoice_id.sale_order_comm_ids.\
                filtered(lambda l: l.commission_type == ptype):
            vals = {
                'name': self.name,
                'sale_id': self.id,
    ...
    def write(self, vals):
        ...
        commission_pay_on = ir_config_obj.sudo().get_param(
            'aspl_sales_commission_target.commission_pay_on')
        commission_pay_by = ir_config_obj.sudo().get_param(
            'aspl_sales_commission_target.commission_pay_by')
        ...
        if commission_pay_on == 'order_confirm':
            for sale_id in self.filtered(lambda sale: sale.state == 'sale'):
                sale_id.order_calculate_commission(ptype='sales_person',
                                                   commission_pay_by=commission_pay_by)

        if cons_commission_pay_on == 'order_confirm':
            for sale_id in self.filtered(lambda sale: sale.state == 'sale'):
                sale_id.order_calculate_commission(ptype='consultant',
                                                   commission_pay_by=cons_commission_pay_by)

        if dist_commission_pay_on == 'order_confirm':
            for sale_id in self.filtered(lambda sale: sale.state == 'sale'):
                sale_id.order_calculate_commission(ptype='distributor',
    ...
    @api.depends('partner_id', 'team_id', 'user_id', 'commission_calc', 'amount_total',
                 'commission_role_ids', 'consultant_ids')
            ...
            if self.commission_calc == 'product':
                for soline in self.order_line:
                    if role.name == 'Sales Person' and commission_pay_on == 'order_confirm':
                        for lineid in soline.product_id.product_comm_ids:
                            ...
                            details = {'name': soline.product_id.display_name,
                                       'person': self.user_id.partner_id.id,
                                       'role': role.id, 'type': lineid.compute_price_type,
                                       'commission': lineid.commission}
                            if (lineid.user_ids and self.user_id.id in
                                [user.id for user in lineid.user_ids]) or \
                                    (lineid.job_id and not lineid.user_ids and
                                     self.user_id.id in self.job_related_users(lineid.job_id)):
                                lines[
                        ...
                        for consultant in self.consultant_ids.filtered(lambda line: line.is_consultant):
                            for lineid in soline.product_id.product_cons_comm_ids:
                                if consultant.id == lineid.partner_id.id:
                                    ...
                                    details_lst.append(
                                        {'name': soline.product_id.display_name,
                        ...
                        for lineid in soline.product_id.product_dist_comm_ids.filtered(
                            ...
                            details_lst.append({'name': soline.product_id.display_name,
                ...
                for soline in self.order_line:
                    if role.name == 'Sales Person' and commission_pay_on == 'order_confirm':
                        for lineid in soline.product_id.categ_id.prod_categ_comm_ids:
                            ...
                            details = {'name': soline.product_id.categ_id.display_name,
                                       'person': self.user_id.partner_id.id, 'role': role.id,
                                       'type': lineid.compute_price_type, 'commission': lineid.commission}
                            if (lineid.user_ids and self.user_id.id in
                                [user.id for user in lineid.user_ids]) or \
                                    (lineid.job_id and not lineid.user_ids and
                                     self.user_id.id in self.job_related_users(
                                        lineid.job_id)):
                                lines[
                                    'commission'] = soline.price_subtotal * lineid.commission / 100 if lineid.compute_price_type == 'per' else lineid.commission * soline.product_uom_qty
                                lines['user_sales_amount'] += soline.price_subtotal
                            ...
                            for lineid in soline.product_id.categ_id.prod_categ_cons_comm_ids:
                                if consultant.id == lineid.partner_id.id:
                                    member_lst.append({'partner_id': consultant.id,
                                                       ...
                                                       'commission_type': 'consultant'})
                                    details_lst.append(
                                        {'name': soline.product_id.categ_id.display_name,
                                         'person': consultant.id,
                                         'role': role.id, 'type': lineid.compute_price_type,
                                         'commission': lineid.commission,
                                         'commission_amount': soline.price_subtotal * lineid.commission / 100 if lineid.compute_price_type == 'per' else lineid.commission * soline.product_uom_qty})
                        ...
                        for lineid in soline.product_id.categ_id.prod_categ_dist_comm_ids.filtered(
                                lambda line: line.partner_id.id == self.partner_id.id):
                            member_lst.append({'partner_id': self.partner_id.id,
                                               'commission': soline.price_subtotal * lineid.commission / 100 if lineid.compute_price_type == 'per' else lineid.commission * soline.product_uom_qty,
                                               'commission_type': 'distributor'})
                ...
                if role.name == 'Sales Person' and commission_pay_on == 'order_confirm':
                    for lineid in self.partner_id.comm_ids:
                        lines = {'user_id': self.user_id.id, 'job_id': emp_id.job_id.id,
                                 'commission_type': 'sales_person'}
                        details = {'name': self.partner_id.name,
                                   'person': self.user_id.partner_id.id, 'role': role.id,
                                   'type': lineid.compute_price_type,
                                   'commission': lineid.commission}
                        if (lineid.user_ids and self.user_id.id in
                    ...
                    for consultant in self.consultant_ids.filtered(lambda line: line.is_consultant):
                        for lineid in self.partner_id.consultant_comm_ids:
                            if consultant.id == lineid.partner_id.id:
                                lines = {'partner_id': consultant.id,
                                         'commission_type': 'consultant'}
                                ...
                                member_lst.append(lines)
                                details_lst.append(
                                    {'name': self.partner_id.name, 'person': consultant.id,
                                     'role': role.id,
                                     'type': lineid.compute_price_type,
                                     'commission': lineid.commission,
                    ...
                    for lineid in self.partner_id.distributor_comm_ids.filtered(
                            lambda line: line.partner_id.id == self.partner_id.id):
                        lines = {'partner_id': self.partner_id.id,
                                 'commission_type': 'distributor'}
                        if dist_comm_inc_amount == 'with_tax':
                            ...
                            lines['commission'] = self.amount_untaxed * lineid.commission / 100 if lineid.compute_price_type == 'per' else lineid.commission
                        member_lst.append(lines)
                        details_lst.append({'name': self.partner_id.name,
                                            'person': self.partner_id.id, 'role': role.id,
                                            'type': lineid.compute_price_type,
                                            'commission': lineid.commission,
                                            'commission_amount': lines['commission']})
                        break
                ...
                if role.name == 'Sales Person' and commission_pay_on == 'order_confirm':
                    ...
                    for lineid in self.team_id.sale_team_dist_comm_ids.filtered(
                            lambda line: line.partner_id.id == self.partner_id.id):
                        lines = {'partner_id': self.partner_id.id,
                                 'commission_type': 'distributor'}
                        if dist_comm_inc_amount == 'with_tax':
                        ...
                        member_lst.append(lines)
                        details_lst.append(
                            {'name': self.team_id.display_name, 'person': self.partner_id.id,
                             'role': role.id,
                             'type': lineid.compute_price_type, 'commission': lineid.commission,
                             'commission_amount': lines['commission']})
                        break
    ...
    sale_order_comm_ids = fields.One2many('sales.order.commission', 'order_id',
                                          string="Sale Order Commission",
                                          compute="_compute_commission_data",
                                          store=True)
    details_ids = fields.One2many('commission.calculation.details', 'so_order_id',
                                  string="Details")
    commission_calc = fields.Selection([('sale_team', 'Sales Team'), ('customer', 'Customer'),
                                       ...
                                       copy=False)
    commission_role_ids = fields.Many2many('sales.commission.role', string="Commission Role(s)",
                                           default=default_commission_role_id)
    consultant_ids = fields.Many2many('res.partner', string="Consultants",
