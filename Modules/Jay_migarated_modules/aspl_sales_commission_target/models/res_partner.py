...
class ResPartner(models.Model):
    ...
    def _check_commission_values(self):
        if self.comm_ids.filtered(lambda line: line.compute_price_type == 'per'
                and line.commission > 100 or line.commission < 0.0):
            raise UserError(_('Commission value for Percentage type must be between 0 to 100.'))
        if self.distributor_comm_ids.filtered(lambda line: line.compute_price_type == 'per'
                and line.commission > 100 or line.commission < 0.0):
            raise UserError(_('Commission value for Percentage type must be between 0 to 100.'))
        if self.consultant_comm_ids.filtered(lambda line: line.compute_price_type == 'per'
                and line.commission > 100 or line.commission < 0.0):
            raise UserError(_('Commission value for Percentage type must be between 0 to 100.'))
    ...
    distributor_comm_ids = fields.One2many('distributor.commission', 'cons_id',
                                           string="Distributor Commission")
    consultant_comm_ids = fields.One2many('consultant.commission', 'cons_id',
        ...
        if self.commission and self.compute_price_type == 'per' and \
            ...
            raise UserError(
                _('Entered Commission is %s. \n Commission value '
                  'for Percentage type must be between 0 to 100. ') % self.commission)
...
class ResUsers(models.Model):
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        ...
        elif isinstance(self._context.get('ctx_job_ids'), list) and len(self._context.get('ctx_job_ids')) > 0 and \
                self._context.get('ctx_job_ids')[0]:
            # elif self._context.get('ctx_job_ids', False) and self._context.get('ctx_job_ids')[0][2]:
            emp_ids = self.env['hr.employee'].search([('user_id', '!=', False),
                                                      ('job_id', '=', self._context['ctx_job_ids'])])
            args += [('id', 'in', [emp.user_id.id for emp in emp_ids])]
