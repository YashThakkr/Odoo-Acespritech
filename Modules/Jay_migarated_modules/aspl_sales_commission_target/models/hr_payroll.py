...
class HrPayslip(models.Model):
    ...
    def compute_sheet(self):
        ...
        for payslip in self:
            ...
            if payslip.employee_id.user_id:
                comm_ids = comm_obj.\
                    search(['|', ('user_id', '=', payslip.employee_id.user_id.id),
                ...
                for commline in comm_ids:
                    ...
                    if commline.type == 'sales_person':
                        if not user_dict.get(commline.user_id.id):
                            user_dict.update({commline.user_id.id: {'target': {},
                                                                    'without_target': []}})
                        target_line_id = target_line_obj.\
                            search([('target_id.state', 'in', ['confirmed', 'closed']),
                    ...
                    # consultant and distributor commission line
                        ...
                        if not user_dict.get(commline.cons_id.id):
                            user_dict.update({commline.cons_id.id: {'target': {},
                                                                    'without_target': []}})
                ...
                for vals in user_dict.values():
                    ...
                    for t_lineid, comm_ids in vals.get('target').items():
                        ...
                        if (total_sales_amount >= t_lineid.target_amount) \
                                or payslip.override_target:
    ...
    def action_payslip_done(self):
        ...
        comm_obj = self.env['sales.commission']
        comm_rule_id = self.env.ref('aspl_sales_commission_target.'
                                    'hr_salary_rule_aspl_sales_commission_target')
        if comm_rule_id:
            for payslip in self:
                if payslip.employee_id.user_id and comm_rule_id.id \
                        in [line.salary_rule_id.id for line in payslip.line_ids]:
                    payslip.commission_line_ids.write(
                        {'state': 'paid', 'when_paid': datetime.now(), 'payslip_id': payslip.id})
        return res
...
class HrEmployee(models.Model):
    ...
    def commission_graph(self):
        if self.user_id:
            comm_ids = self.env['sales.commission'].search\
                (['|', ('cons_id', '=', self.user_id.partner_id.id),
