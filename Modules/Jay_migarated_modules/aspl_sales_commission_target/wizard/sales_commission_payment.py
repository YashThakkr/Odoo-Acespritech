...
class SalesCommissionPayment(models.TransientModel):
    ...
    def generate_invoice(self):
        ...
        if not journal_id:
            raise UserError(_('Account Journal not found.'))

        # IrDefault = self.env['ir.config_parameters'].sudo()
        ...
        if self.type == 'sales_person':
            ...
            commission_account_id = self.env['ir.config_parameter'].sudo().get_param(
                'aspl_sales_commission_target.sales_person_commission_account_id')
        elif self.type == 'consultant':
            partner_id = self.consultant_id.id
            # commission_account_id = IrDefault._get('res.config.settings',
            #                                        "consultant_commission_account_id",
            #                                        company_id=self.env.user.company_id.id)
            commission_account_id = self.env['ir.config_parameter'].sudo().get_param(
                'aspl_sales_commission_target.consultant_commission_account_id')
            ...
            #                                        company_id=self.env.user.company_id.id)
            commission_account_id = self.env['ir.config_parameter'].sudo().get_param(
                'aspl_sales_commission_target.distributor_commission_account_id')

        if not commission_account_id:
            raise UserError(
                _('Commission Account is not Found. Please go to Accounting-> '
                  'Configuration and set the Sales commission account.'))
        else:
        ...
        if account_id:
            if self.type in ['consultant', 'distributor']:
                ...
                if inv_line_data:
                    ...
                    invoice_id = invoice_obj.search(
                        [('partner_id', '=', partner_id), ('state', '=', 'draft'),
                         ('move_type', '=', 'in_invoice'), ('commission_invoice', '=', True),
                         ('company_id', '=', self.env.user.company_id.id)])
                    if invoice_id:
                        invoice_id.write({'invoice_line_ids': inv_line_data,
                                          'commission_invoice': True})
                    else:
                ...
                if invoice_id:
                    ...
                    raise UserError("No Invoice found or Generated.")
                ...
                for commline in commission_ids:
                    if not user_dict.get(commline.user_id.id):
                        user_dict.update({commline.user_id.id: {'target': {},
                                                                'without_target': []}})
                    ...
                    if target_line_id:
                        if (target_line_id.target_state == 'open') \
                                or (target_line_id.start_date <= fields.Date.today()
                                    <= target_line_id.end_date):
                            continue
                ...
                for user, vals in user_dict.items():
                    ...
                    if inv_line_data:
                        ...
                        for commid in commission_lines:
                            commid.write({'invoice_id': invoice_id.id,
                                          'state': 'invoiced'})
                if invoice_id:
                    view_id = self.env.ref("account.view_move_form")
                    return {
    ...
    override_target = fields.Boolean(string='Override Target',
                                     help="If checked, then it will override"
                                          " user's Commission Target.")
...
class wizard_commission_summary(models.TransientModel):
    ...
    consultant_ids = fields.Many2many('res.partner', 'consultant_partner_rel',
                                      string="Consultant(s)",
                                      domain=[('is_consultant', '=', True)])
    distributor_ids = fields.Many2many('res.partner', 'distributor_partner_rel',
                                       string="Distributor(s)",
                                       domain=[('is_distributor', '=', True)])
    status = fields.Selection([('paid', 'Paid'), ('unpaid', 'Unpaid'),
                               ('cancel', 'Cancel'), ('all', 'All')],
                              string="Status", default='paid', required=True)
    template_id = fields.Many2one('mail.template', string="Mail Template")
    mail_to = fields.Many2many('res.partner', string='Mail To')
    type = fields.Selection([('sales_person', 'Sales Person'), ('consultant', 'Consultant'),
                             ('distributor', 'Distributor'), ('all', 'All')], string="Type",
                            default='sales_person', required=True)
    ...
    def get_users_commission(self):
        ...
        consultant_ids = [user.id for user in self.consultant_ids or
                          self.env['res.partner'].search([('is_consultant', '=', True)])]
        distributor_ids = [user.id for user in self.distributor_ids or
        ...
        for commid in self.env['sales.commission'].search(domain, order="commission_date"):
            ...
            key = 'sales_person_' + str(commid.user_id.id) if commid.type == 'sales_person' else \
                'cons_' + str(commid.cons_id.id)
            result[key].append(vals) if key in result else result.update({key: [vals]})
        if not result:
            raise UserError(_('Sales Commission Details not found.'))
        return result
    ...
    def user_summary(self):
        ...
        for commid in self.env['sales.commission'].search(domain, order="commission_date"):
            ...
            if user_name in result:
                for res in [result[user_name]]:
                    result[user_name] += round(amount, 2)
            else:
                result.update({user_name: round(amount, 2)})
    ...
    def print_commission_report(self):
        ...
        datas = {
            ...
            'summary_details': self.user_summary()
        }
        return self.env.ref('aspl_sales_commission_target.report_print_commission_summary') \
    ...
    def send_commission_report(self):
        ...
        if self.mail_to and self.template_id:
            ...
            pdf_data = self.env["ir.actions.report"].sudo()._render_qweb_pdf(
                'aspl_sales_commission_target.report_print_commission_summary', self, data=datas)
            if pdf_data:
            ...
            raise UserError(_('Email id is not defined to send email. Please Enter Email id !'))
        elif not self.template_id:
            raise UserError(_('Please Select Email template !'))
