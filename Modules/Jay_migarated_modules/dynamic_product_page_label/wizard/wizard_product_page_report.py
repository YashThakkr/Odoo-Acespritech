...
paper_format_dict = {
    ...
    'B8': [62, 88], 'B9': [33, 62], 'B10': [31, 44],
    'C5E': [163, 229], 'Comm10E': [105, 241],
    'DLE': [110, 220],
    'Executive': [190.5, 254],
    'Folio': [210, 330],
    'Legal': [215.9, 355.6], 'Letter': [215.9, 279.4], 'Tabloid': [279.4, 431.8],
    'Ledger': [431.8, 279.4],
}
...
class wizard_product_page_report(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if self._context.get('active_ids') and self._context.get('active_model') == 'purchase.order':
            ...
            for each_lot in self.env['stock.lot'].search([('id', 'in', self._context.get('active_ids'))]):
                if each_lot.product_id and each_lot.product_qty:
                    product_list_dict.append({'product_id': each_lot.product_id.id, 'qty': each_lot.product_qty, 'line_id':each_lot.id})
    ...
    def _get_report_paperformat_id(self):
        xml_id = self.env['ir.actions.report'].search([('report_name', '=', 'dynamic_product_page_label.dynamic_prod_page_rpt')])
    ...
    def action_print(self):
        ...
        return self.env.ref('dynamic_product_page_label.dynamic_product_page_report').report_action(self, data=datas)

    def save_design(self):
        if not self.make_update_existing:
            view_id = self.env.ref('dynamic_product_page_label.wizard_page_label_design_form_view').id
