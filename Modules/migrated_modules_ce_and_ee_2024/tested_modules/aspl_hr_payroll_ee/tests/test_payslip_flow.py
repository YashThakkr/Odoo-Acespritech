...
from odoo.addons.aspl_hr_payroll.tests.common import TestPayslipBase
...
class TestPayslipFlow(TestPayslipBase):
    ...
    def test_00_payslip_flow(self):
        ...
        context = {
            ...
            "active_ids": [self.ref("aspl_hr_payroll_ee.menu_department_tree")],
            "active_id": self.ref("aspl_hr_payroll_ee.menu_department_tree")
        }
        ...
        data, data_format = self.env.ref('aspl_hr_payroll_ee.action_report_payslip').render(richard_payslip.ids)

        # I print the payslip details report
        data, data_format = self.env.ref('aspl_hr_payroll_ee.payslip_details_report').render(richard_payslip.ids)

        # I print the contribution register report
        context = {'model': 'hr.contribution.register', 'active_ids': [self.ref('aspl_hr_payroll_ee.hr_houserent_register')]}
        test_reports.try_report_action(self.env.cr, self.env.uid, 'action_payslip_lines_contribution_register', context=context, our_module='aspl_hr_payroll_ee')
