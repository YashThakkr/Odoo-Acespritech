   ...
   function _findMobileView(views, multiRecord) {
    ...
    async function _executeReportAction(action, options) {
        ...
        if (action.report_type === "qweb-html") {
            ...
            const rpcDataForName = await this.orm.call(
                    'ir.actions.report',
                    'print_action_for_report_name',
                    [[action.report_name], action.data],
                );
            if(rpcDataForName && rpcDataForName.action === 'server'){
//                framework.unblockUI();
//                const printDocument = await rpc.query({
//                    model: 'ir.actions.report',
//                    method: 'print_document',
//                    args: [action.context.active_ids,
//                            action.report_name, 'none',
//                            action.data,  { context: action.context || {} }],
//                });
                console.log("RPC DATA-----", rpcDataForName)
...
//                        params: {
//                            'title': _('Report'),
//                            'message': _('Document sent to the printer ') + rpcDataForName.printer_name,
//                            'type':'info',
//                            'sticky': false,
//                        }
//                    });
    ...
    async function custom(action, options){
        const handlers = registry.category("ir.actions.report handlers").getAll();
        for (const handler of handlers) {
            const result = await handler(action, options, env);
            if (result) {
    ...
    async function customDoActionButton(params){
        // determine the action to execute according to the params
        let action;
        const context = makeContext([params.context, params.buttonContext]);
        if (params.special) {
