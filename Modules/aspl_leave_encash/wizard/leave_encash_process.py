...
class LeaveEncashProcess(models.TransientModel):
    ...
    def create_encashment(self):
        ...
        if self.encashment_type:
            ...
            warnings.warn(
            "Please select Encashment Policy.",
            DeprecationWarning,
            stacklevel=2
        )
    ...
    def search_leave(self):
        ...
        employee_ids = self.env['hr.employee'].sudo().search([('department_id', 'in', dept_ids),
                                                               ('job_id', 'in', job_ids)])
        leave_encash_settings = self.env['res.config.settings'].sudo().search([], limit=1, order='id DESC')
        list = []
...
class LeaveEncashLines(models.TransientModel):
    ...
    def _check_max_term(self):
        for each in self:
            if each.encash_leave != 0 and each.encash_leave > each.pending_leave:
