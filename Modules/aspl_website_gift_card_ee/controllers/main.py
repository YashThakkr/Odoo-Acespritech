...
class WebsiteSale(WebsiteSale):
            ...
            if not gift_card_session_id:
                ...
                if sale_order.amount_total >= amount:
                    ...
                    product_id = int(request.env['ir.config_parameter'].sudo().get_param(
                        'aspl_website_gift_card_ee.product_id'))
                    data = [(0, 0, {
                        'name': 'Gift Card',
    ...
    def cancel_gift_card(self, **kw):
        ...
        product_id = int(request.env['ir.config_parameter'].sudo().get_param('aspl_website_gift_card_ee.product_id'))
    ...
    def cart_update_json(self, product_id, line_id=None, add_qty=None, set_qty=None, display=True, **kw):
        ...
        setting_product_id = int(request.env['ir.config_parameter'].sudo().get_param('aspl_website_gift_card_ee.product_id'))
        sale_order_id = request.session.get('sale_last_order_id')
        order = request.env['sale.order'].sudo().browse(sale_order_id)
        line = order.order_line.filtered(lambda r: r.product_id.is_gift_card)
