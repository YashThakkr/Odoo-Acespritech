...
class SubscriberPlan(models.Model):
    ...
    def view_subscriber_plan_invoices(self):
        action = self.env.ref('aspl_fitness_management.act_subscriber_plan_2_invoice').read()[0]
...
class ResPartner(models.Model):
    ...
    def get_action_subscriber_plan_tree(self):
        ...
        action = \
            self.env.ref('aspl_fitness_management.action_subscriber_to_subscriber_plan').read()[0]
        if len(subscriber_plans) > 1:
            action['domain'] = [('id', 'in', subscriber_plans.ids)]
        elif len(subscriber_plans.ids) == 1:
            action['views'] = [
                [self.env.ref('aspl_fitness_management.view_subscriber_plan_form').id, 'form'],
                [self.env.ref('aspl_fitness_management.view_subscriber_plan_tree').id, 'tree']]
