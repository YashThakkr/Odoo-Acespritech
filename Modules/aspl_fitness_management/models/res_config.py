...
class ResConfig(models.TransientModel):
    ...
    def get_values(self):
        ...
        nutrition_management = config_param_obj('aspl_fitness_management.nutrition_management')
        feedback = config_param_obj('aspl_fitness_management.feedback')
        welcome_mail_tmp_id = int(
            config_param_obj('aspl_fitness_management.welcome_mail_tmp_id'))
        invoice_mail_tmp_id = int(
            config_param_obj('aspl_fitness_management.invoice_mail_tmp_id'))
        membership_renewal_days = int(
            config_param_obj('aspl_fitness_management.membership_renewal_days'))
        expiry_alert_days = int(
            config_param_obj('aspl_fitness_management.expiry_alert_days'))
    ...
    def set_values(self):
        ...
        config_param_obj.set_param("aspl_fitness_management.nutrition_management",
                                   self.nutrition_management)
        config_param_obj.set_param("aspl_fitness_management.feedback", self.feedback)
        config_param_obj.set_param("aspl_fitness_management.welcome_mail_tmp_id",
                         self.welcome_mail_tmp_id.id if self.welcome_mail_tmp_id else False)
        config_param_obj.set_param("aspl_fitness_management.invoice_mail_tmp_id",
                         self.invoice_mail_tmp_id.id if self.invoice_mail_tmp_id else False)
        config_param_obj.set_param("aspl_fitness_management.membership_renewal_days",
                         self.membership_renewal_days)
        config_param_obj.set_param("aspl_fitness_management.expiry_alert_days",
                         self.expiry_alert_days)

        group_id = self.env['res.groups'].browse(
            self.env.ref('aspl_fitness_management.nutrition_security_group').id)
