...
class GradeExpenseConfigure(models.Model):
    ...
    def find_ids(self):
        ...
        if dupes:
            raise ValidationError(_("Please remove duplicate expenses."))
    ...
    def compute_total(self):
        for each in self:
            if each.expense_line_ids:
                each.total = sum([i.amount if i else 0 for i in each.expense_line_ids])
            else:
                each.total = 0
...
class HrEmployeeExpenses(models.Model):
    ...
    def check_before_transaction(self):
        if self.travel_request_id.name:
            ...
            raise ValidationError(_("Please Save the Travel Request."))
