...
import { Component, markup, onMounted, onWillUnmount, onError, useChildSubEnv, xml, reactive, hooks, } from "@odoo/owl";
const { useRef, useSubEnv } = "@web/core/utils/hooks";
...
//                cleanDomFromBootstrap();
        ...
        // ControllerComponent.template = 'aspl_product_small_label_zebra.prod_small_label_zebra';
   ...
   function _findMobileView(views, multiRecord) {
    ...
    async function doActionButton(params) {
        ...
        if (params.special) {
            ...
            if (params.name === "show_map") {
                console.log('\n\n params.name---', params.name)
                var def = $.Deferred();
                return rpc.query({
                    model: 'partner.search.map',
                    method: 'show_map',
                    args: [params.resId],
                }, {
                    async: false
                    ...
                    if (result && result[0]) {
                        ...
                        for (var i = 0; i < result.length; i++) {
                            ...
                            if (result[i].latitude && result[i].longitude) {
                                if (lat_long.length > 0) {
                                    _.each(lat_long, function(item) {
                                        if ($.inArray(result[i].latitude, item) !== -1 && $.inArray(result[i].longitude, item) !== -1) {
                                            item[0] += "<b>" + result[i].name + "</b>" + "<br/>";
                                            if (result[i].image) {
                                                item[0] += "<img src='data:image/png;base64," + result[i].image + "' width='50' height='50' /> <br/>"
                                            }
                                            if (result[i].street || result[i].street2 ||
                                                result[i].city || result[i].zip ||
                                                result[i].state || result[i].country) {
                                                item[0] += "<b>Address :</b>" + result[i].street ? result[i].street : '' + ",<br/>"
                                                item[0] += result[i].street2 ? result[i].street2 : '' + ",<br/>"
                                                item[0] += result[i].city ? result[i].city : '' + ","
                                                item[0] += result[i].zip ? result[i].zip : '' + ",<br/>"
                                                item[0] += result[i].state ? result[i].state : '' + "<br/>"
                                                item[0] += result[i].country ? result[i].country : '' + "<br/>"
                                            }
                                            if (result[i].mobile) {
                                                item[0] += "<b>Mobile No</b>" + " : " + result[i].mobile + "<br/>";
                                            }
                                            item[3] = item[3] + 1;
                                            exit = false;
                                            return;
                                ...
                                if (exit) {
                                    str = "<b>" + result[i].name + "</b>" + "<br/>";
                                    if (result[i].image) {
                                        str += "<img src='data:image/png;base64," + result[i].image + "' width='50' height='50' /> <br/>"
                                    }
                                    if (result[i].street || result[i].street2 ||
                                        result[i].city || result[i].zip ||
                                        result[i].state || result[i].country) {
                                        str += "<b>Address :</b>" + result[i].street + ",<br/>" ? result[i].street : '<br/>'
                                        str += result[i].street2 ? result[i].street2 + ",<br/>" : '<br/>'
                                        str += result[i].city ? result[i].city + ",<br/>" : '<br/>'
                                        str += result[i].zip ? result[i].zip + ",<br/>" : '<br/>'
                                        str += result[i].state ? result[i].state + "<br/>" : '<br/>'
                                        str += result[i].country ? result[i].country + "<br/>" : '<br/>'
                                    }
                                    if (result[i].mobile) {
                                        str += "<b>Mobile No</b>" + " : " + result[i].mobile + "<br/>";
                                    }
                        ...
                        if (lat_long.length > 0) {
                            $('#record').html((result.length - 1) + ' Records Found')
                            ...
                            console.log("\n\n------res", result)
                            if (result && result[1]) {
                                alert("Please select company or contact.")
                            } else {
                                alert("No Record Found")
                            }

                            $('#record').html('0 Records Found')
