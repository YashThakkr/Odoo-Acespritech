...
class report_hr_adv_sal(models.AbstractModel):
    _name = 'report.aspl_hr_advance_salary_ee.template_hr_adv_sal'

    @api.model
    def _get_report_values(self, docids, data=None):
        print("\n\n\n calll")
        return {
            'doc_ids': self.env['summary.wiz'].browse(data['id']),
            'doc_model': 'summary.wiz',
            'docs': self,
    ...
    def summary_emp(self, obj):
        ...
        if obj.view_by == 'employee':
            query = "SELECT employee_id, name, request_date, approved_date, disburse_date, approved_amt, state \
        ...
        if obj.view_by == 'state' and obj.state:
            adv_sal_ids = self.env['hr.advance.salary.request'].search([
                                                ...
                                                ('request_date', '<=', obj.date_to)], order='employee_id')
            return adv_sal_ids
        if obj.view_by == 'month':
            adv_sal_ids = self.env['hr.advance.salary.request'].search([
                                ('employee_id', 'in', emp_ids),
                                ('request_date', '>=', self.get_date(obj)[0]),
                                ('request_date', '<=', self.get_date(obj)[1])], order='employee_id')
            return adv_sal_ids
