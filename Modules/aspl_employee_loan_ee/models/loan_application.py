...
class LoanApplication(models.Model):
    ...
    @api.onchange('app_categ_id')
    def onchange_field(self):
        final_list = []
        if self.app_categ_id:
            if not self.loan_type_doc_ids.ids:
                final_list.append((5, 0, 0))
    ...
    def approved_document(self):
        ...
        if move_line:
            ...
            account_move_obj._post()
        self.write({"state": "verified"})

    def rejected_mail_template(self):
        template_id = self.env.ref('aspl_employee_loan_ee.email_template_for_reject_loan')
    ...
    def cancelled_mail_template(self):
        template_id = self.env.ref('aspl_employee_loan_ee.email_template_for_cancel_loan')
        template_id.send_mail(self.id, force_send=True)
    ...
    def rejected(self):
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'view_id': self.env.ref('aspl_employee_loan_ee.wizard_loan_form_view').id,
    ...
    def verified_approve(self):
        template_id = self.env.ref('aspl_employee_loan_ee.email_template_for_loan_manager')
    ...
    def button_approved(self):
        template_id = self.env.ref('aspl_employee_loan_ee.email_template_for_apporve_loan')
        template_id.send_mail(self.id, force_send=True)
        ...
        for each in self.loan_payment_ids:
            if count <= len_date:
                if lst_date:
                    ...
                    count += 1
        template_id = self.env.ref('aspl_employee_loan_ee.email_template_for_paid_loan')
        template_id.send_mail(self.id, force_send=True)
        ...
        for payment in self:
            ...
            if payment.currency_id:
                payment.update({'total_paid_amt': payment.currency_id.round(paid_amt + pre_payment_amount),
                                'total_remaining_amt': round(payment.currency_id.round(payment.amount) -
                                                             payment.currency_id.round(paid_amt + pre_payment_amount), 2),
    ...
    def compute_payments_again(self):
        if self.loan_type_id and self.loan_type_id.maximum_amount < self.amount:
            raise ValidationError(_('Disbursed Loan amount exceed the limit %d.'
    ...
    def loan_user_url(self):
        ...
        action = self.env.ref('aspl_employee_loan_ee.action_loan_application').id
        view_type = 'form'
        menu_id = self.env.ref('aspl_employee_loan_ee.menu_loan_application_draft').id
    ...
    def loan_close(self):
        ...
        template_obj = self.env.ref('aspl_employee_loan_ee.email_template_loan_close')
        template_obj.send_mail(self.id, force_send=True)

    @api.depends('loan_payment_ids.state')
    def _check_loan_fully_paid(self):
        for record in self:
            all_record = all([x.state == 'paid' for x in record.loan_payment_ids])
            record.fully_paid = all_record
    @api.model
    ...
    def create_loan_calc(self):
        ...
        return {
            ...
            'view_id': self.env.ref('aspl_employee_loan_ee.loan_calc_form').id,
            'target': 'new'
        }
