...
class ResConfigSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        config_param_obj("aspl_hr_overtime_ee.holiday_ot_rate", self.holiday_ot_rate)
        config_param_obj("aspl_hr_overtime_ee.ot_time_difference_limit", self.ot_time_difference_limit)
        config_param_obj("aspl_hr_overtime_ee.manually_create_overtime_req", self.manually_create_overtime_req)
        config_param_obj("aspl_hr_overtime_ee.payslip_date_range", self.payslip_date_range)
        user_ids = self.env['res.users'].search([])
        if self.manually_create_overtime_req:
            group_id = self.env.ref('aspl_hr_overtime_ee.group_overtime_manual_overtime')
            for user in user_ids:
                group_id.write({'users': [(4, user.id)]})
        else:
            group_id = self.env.ref('aspl_hr_overtime_ee.group_overtime_manual_overtime', False)
            for user in user_ids:
